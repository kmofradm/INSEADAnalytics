...
* <b> 1. </b> 6
* <b> 2 </b> 0.93
* <b> 3 </b>  The first 10 components identify different types of financial assets/ markets and group them together. Comp 1 : Europoean and North American Bonds <br> Comp 2 : Safe Haven Currencies <br> Comp 3: Large European Stock Exchanges   <br> Comp 4 : Smaller Norht American Stock Exchanges <br> Comp 5 : European Financial Assets <br> Comp 6: Oil & Gas <br> Comp 7 : Regular Metal <br> Comp 8 : Agricultural Commodities <br> Comp 9 : Precious Metal <br> Comp 10 : Asian Stock Exchanges
**<b> 4</b> 
```{r echo=TRUE, evol=TRUE}
crisis_data = futures_data [as.Date(rownames(futures_data))>"2007-10-01" & as.Date(rownames(futures_data)) < "2009-03-01", ]
precrisis_data = futures_data [as.Date(rownames(futures_data)) >"2002-10-01" & as.Date(rownames(futures_data)) < "2007-10-01 " , ]

```
<br>

<br> <b> Crisis Period <b> <br>
```{r echo=TRUE, evol=TRUE}
...
# Here is the table and the plot of bear years
show_data_bear = round(Variance_Explained_Table_crisis, 2)
iprint.df(show_data_bear)
...
<br> <b> Pre Crisis Period <b> <br>

```{r echo=TRUE, evol=TRUE}
Variance_Explained_Table_results_precrisis<-PCA(precrisis_data, graph=FALSE)
Variance_Explained_Table_precrisis<-Variance_Explained_Table_results_precrisis$eig
colnames(Variance_Explained_Table_precrisis)<-c("Eigenvalue", "Pct of explained variance", "Cumulative pct of explained variance")

# Here is the table and the plot of bull years

show_data_bull= round(Variance_Explained_Table_precrisis, 2)
iprint.df(show_data_bull)
eigenvalues  <- Variance_Explained_Table_precrisis[, "Eigenvalue"]
df           <- cbind(as.data.frame(eigenvalues), c(1:length(eigenvalues)), rep(1, length(eigenvalues)))
colnames(df) <- c("eigenvalues", "components", "abline")
iplot.df(melt(df, id="components"))
```

<br> The elements are more correlated during crisis and the scree plot is steeper. So, less number of principal components have Eigenvalues higher than one during the crisis.  
*
*
*
*
...
*1. The code calculates moving averages of the future prices in different time windows. After that, it scales the price down so that there is $1/day investment across all futures. 
2. We can change the code in line 194, for example : MAfreq<-seq(5,250,by=20)

```{r echo=TRUE, tidy=TRUE}
signal_used = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq<-seq(5,250,by=20)
for (iter in 1:length(MAfreq))
  signal_used = signal_used + sign(apply(futures_data,2, function(r) ma(r,MAfreq[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used = t(apply(signal_used,1,function(r) {
